	...
	$("#btnCheckDupId").click(function() {
		...
		$.get(url, sendData, function(rData){
			...
			if(rData == "true"){
				...
				$("#checkDupIdResult").text("사용가능한 아이디").css("color", "blue");
				checkId = user_id;
			}
			isCheckDupId = true;
		});
...
<div class="container-fluid">
	<div class="row">
		<div class="col-md-12">
			...
			<form role="form" id="frmJoin" action="/memberJoinRun" method="post"
				enctype="multipart/form-data">
				<div class="form-group">

					<label for="user_id"> 아이디 </label> <input type="text"
						class="form-control" id="user_id" name="user_id" required /> <br />
					<button type="button" class="btn btn-small btn-danger"
						id="btnCheckDupId">아이디 중복 확인</button>
					<span id="checkDupIdResult"></span>
				</div>
				<div class="form-group">

					<label for="user_pw"> 패스워드 </label> <input type="password"
						class="form-control" id="user_pw" name="user_pw" required />
				</div>
				<div class="form-group">

					<label for="user_pw"> 패스워드 확인</label> <input type="password"
						class="form-control" id="user_pw2" required />
				</div>

...
public class EmpFrame extends JFrame implements ActionListener{
	...
	String[] items = { "사원명", "직급", "부서번호" };
	String[] colNames = {"ename", "job", "deptno"};
	JComboBox<String> combo = new JComboBox<String>(items);
	JTextField txtKeyword = new JTextField(5);
	...
	private void setUI() {
		for(int i=0; i<attribute.length; i++) {
			...
			lbl[i].setHorizontalAlignment(SwingConstants.RIGHT); // 라벨 중앙정렬
			tf[i] = new JTextField();
			pInput.add(lbl[i]);
			pInput.add(tf[i]);
		}
	...
	private void printListData(List<EmpVo> list) {
		ta.setText("");
	...
	public void actionPerformed(ActionEvent e) {
		...
		} else if(e.getSource().equals(btnSearch)) { // "검색" 버튼
			//String item = (String) combo.getSelectedItem();
			int index = combo.getSelectedIndex();
			String colName = colNames[index];
			String keyword = txtKeyword.getText();
			//System.out.println(colName + keyword);
			List<EmpVo> list = empDao.searchData(colName, keyword);
